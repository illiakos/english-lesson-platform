...
/* ── Multi-prompt roleplay / short answers ─────────────────────────────────── */

function PromptedWritingTask({ section, onComplete }: WritingTaskProps) {
  ...
  const allFilled =
    prompts.length > 0 &&
    prompts.every((_, i) => (answers[i] ?? '').trim().length > 0)

  const copy = () => {
    const text = prompts
      .map((_, i) => `${i + 1}. ${(answers[i] ?? '').trim()}`)
      .join('\n')
      ...
      {section.wordBank && section.wordBank.length > 0 && (
        <div>
          ...
          <p className="mt-1.5 text-xs text-slate-400">
            Focus a box, then tap a phrase to insert — or tap a phrase to add to prompt 1.
          </p>
        </div>
      )}
        ...
        {prompts.map((label, i) => (
          <label key={i} className="block">
            <span className="mb-1.5 flex items-center gap-2 text-xs font-bold text-slate-600">
              <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-orange-100 text-[11px] font-bold text-orange-700">
                {i + 1}
              </span>
              {label}
            </span>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {onComplete && (
          <button
            type="button"
            onClick={() => {
              if (allFilled) onComplete(section.id)
            }}
            disabled={!allFilled}
...
export default function WritingTask({ section, onComplete }: WritingTaskProps) {
  if (section.prompts && section.prompts.length > 0) {
    return <PromptedWritingTask section={section} onComplete={onComplete} />
  }
